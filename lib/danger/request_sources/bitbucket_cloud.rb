  ...
  module RequestSources
    class BitbucketCloud < RequestSource
      ...
      def setup_danger_branches
        ...
        puts scm.ensure_commitish_exists! base_commit
        puts self.scm.exec "branch #{EnvironmentManager.danger_base_branch} #{base_commit}"

        # OK, so we want to ensure that we have a known head branch, this will always represent
        # the head of the PR ( e.g. the most recent commit that will be merged. )
        puts scm.ensure_commitish_exists! head_commit
        puts self.scm.exec "branch #{EnvironmentManager.danger_head_branch} #{head_commit}"
      end
